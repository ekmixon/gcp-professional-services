...
from tensorflow.python.training.basic_session_run_hooks import _HookTimer
...
class LambdaTriggerTimer(_HookTimer):
    """Timer that triggers according to a lambda function that
        depends on a step."""
    def __init__(self, steps_lambda):
        self._last_triggered_step = None
        self.steps_lambda = steps_lambda
        super(LambdaTriggerTimer, self).__init__()
...
class LambdaProfilerHook(tf.train.ProfilerHook):
    def __init__(self,
                 steps_lambda,
                 output_dir="",
                 show_dataflow=True,
                 show_memory=False,):
        super(LambdaProfilerHook, self).__init__(output_dir=output_dir,
                                                 save_secs=3600,
                                                 show_dataflow=show_dataflow,
                                                 show_memory=show_memory)
        self._timer = LambdaTriggerTimer(steps_lambda)
...
def train_and_evaluate(output_dir):
    ...
    estimator = tf.estimator.DNNLinearCombinedRegressor(
        ...
        config=run_config)
    hook = LambdaProfilerHook(
        steps_lambda=lambda step: step in [10000, 10001, 10002],
